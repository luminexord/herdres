...
def invalidate_cached_pane_turn(pane_id: str) -> None:
    clean_pane_id = str(pane_id or "").strip()
    if not clean_pane_id:
        return
    with _turn_cache_lock:
        _turn_cache.pop(clean_pane_id, None)


def fresh_cached_pane_turn(pane_id: str) -> dict[str, Any]:
    clean_pane_id = str(pane_id or "").strip()
    if not clean_pane_id:
        return {}
    invalidate_cached_pane_turn(clean_pane_id)
    return pane_turn(clean_pane_id)
...
def _worker_key_component(value: Any, *, fallback: str = "worker") -> str:
    text = sanitize_text(str(value or ""), 120).strip()
    text = re.sub(r"[^A-Za-z0-9_.:-]+", "-", text).strip(".:-_")
...
def pane_key(pane: dict[str, Any]) -> str:
    ...
    if str(pane.get("source") or "") == "tendwire" and str(pane.get("entry_type") or "") == "worker":
        ...
        parts = [
            worker_id,
            str(pane.get("space_id") or pane.get("workspace_id") or ""),
            str(pane.get("tab_id") or ""),
        ]
        raw = "|".join(parts)
        digest = hashlib.sha1(raw.encode("utf-8")).hexdigest()[:10]
        return f"worker:{_worker_key_component(worker_id)}:{digest}"
    parts = [
...
def space_key(pane: dict[str, Any]) -> str:
    if per_agent_topics_enabled():
        ...
        if str(pane.get("source") or "") == "tendwire" and str(pane.get("entry_type") or "") == "worker":
            worker_id = _space_component(str(pane.get("worker_id") or pane.get("_tendwire_worker_id") or ""))
            if worker_id:
                return f"agent:worker:{worker_id}"
...
COUNCIL_LABEL_RE = re.compile(r"^\s*council[-\s]+([A-Za-z0-9_.-]+)", re.IGNORECASE)
...
def council_seat_display_label(seat_slug: str) -> str:
    slug = clean_council_seat_slug(seat_slug)
    if not slug:
        return ""
    words = [
        TOPIC_TITLE_ACRONYMS.get(part, part.title())
        for part in slug.split("-")
        if part
    ]
    return sanitize_text("Council " + (" ".join(words) or slug.title()), 80)


def council_display_label_for_entry_like(entry: dict[str, Any], pane: dict[str, Any] | None = None) -> str:
    slug = council_seat_slug_from_entry_like(entry, pane)
    return council_seat_display_label(slug)


def is_gitmoot_workflow_event(text_or_item) -> bool:
    if isinstance(text_or_item, dict):
        for key in ("event", "type", "kind", "action", "name", "event_type"):
            value = clean_council_seat_slug(str(text_or_item.get(key) or "")).replace("-", "_")
            if value in GITMOOT_WORKFLOW_EVENTS:
                return True
        message = text_or_item.get("message") or text_or_item.get("text")
        return isinstance(message, str) and is_gitmoot_workflow_event(message)
    text = ANSI_RE.sub("", str(text_or_item or "")).strip()
    if not text:
        return False
    if text[:1] in "{[":
        try:
        ...
        if isinstance(parsed, dict):
            return is_gitmoot_workflow_event(parsed)
    event_alt = "|".join(re.escape(event) for event in sorted(GITMOOT_WORKFLOW_EVENTS))
    return bool(
        re.match(rf"^(?:{event_alt})(?:\b|[\s:])", text)
        or re.match(rf"^(?:gitmoot|workflow|lifecycle|delegation)[\s:_-]+(?:event[\s:_-]+)?(?:{event_alt})(?:\b|[\s:])", text, re.IGNORECASE)
        or re.search(rf'"(?:event|type|kind|action|name|event_type)"\s*:\s*"(?:{event_alt})"', text)
    )
...
def gitmoot_delegation_ref_from_pane(pane) -> tuple[str, str] | None:
    ...
    indexes = [idx for idx, part in enumerate(parts) if part == "delegations"]
    if len(indexes) != 1:
        return None
    idx = indexes[0]
    if idx + 2 >= len(parts):
    ...
    if (
        label_match
        and delegation_id.lower().startswith("d")
        and delegation_id[1:].isdigit()
        and delegation_id.lower() != f"d{int(label_match.group(1))}"
    ):
        return None
...
def gitmoot_council_feed_item(pane: dict[str, Any], entry: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if str(entry.get("last_council_job_ref") or "") == marker:
        return None
    item = make_turn_feed_item({
        "available": True,
        "complete": True,
        "has_open_turn": False,
        "assistant_final_text": content,
        "turn_id": f"gitmoot:{marker}",
    })
    if item:
        item["_council_job_ref"] = marker
    return item
...
def space_name_for_pane(pane: dict[str, Any]) -> str:
    ...
    raw_name = str(pane.get("space_name") or pane.get("workspace_label") or pane.get("workspace_name") or "")
    explicit_name = "" if _looks_like_raw_herdr_space_id(raw_name) else clean_space_topic_title(raw_name, fallback="")
    if explicit_name:
        return explicit_name
    raw_space_id = str(pane.get("space_id") or "")
    explicit_space = "" if _looks_like_raw_herdr_space_id(raw_space_id) else clean_label_topic_title(raw_space_id, fallback="")
    if explicit_space:
        return explicit_space
    raw_workspace_id = str(pane.get("workspace_id") or pane.get("workspace") or "")
    workspace = "" if _looks_like_raw_herdr_space_id(raw_workspace_id) else clean_label_topic_title(raw_workspace_id, fallback="")
    if workspace:
        return workspace
    cwd_value = str(pane.get("foreground_cwd") or pane.get("cwd") or "")
...
def ensure_space_entry(state: dict[str, Any], pane: dict[str, Any]) -> tuple[str, dict[str, Any], bool]:
    ...
    if not current_topic_name:
        ...
        changed = True
    elif (workspace_label or council or _looks_like_raw_herdr_space_id(current_topic_name)) and current_topic_name != topic_name:
        entry["topic_name"] = topic_name
...
def status_object(pane: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "agent": council_label or str(pane.get("agent") or ""),
        ...
        "label": council_label or sanitize_text(str(pane.get("label") or ""), 120),
    }
...
def _direct_origin_origin_allows(origin: Any) -> bool:
    if isinstance(origin, dict):
        origin = origin.get("direct_origin_origin")
    return str(origin or "").strip().lower() in DIRECT_ORIGIN_ALLOWED_ORIGINS


def _direct_origin_user_text_hash(user_text: str) -> str:
    clean = sanitize_text(str(user_text or ""), USER_PROMPT_MAX_CHARS).strip()
...
def _direct_origin_marker_expired(entry: dict[str, Any]) -> bool:
    if not any(key in entry for key in DIRECT_ORIGIN_MARKER_FIELDS):
        return False
    age = _iso_age_seconds(str(entry.get("direct_origin_at") or entry.get("direct_origin_created_at") or ""))
...
def _direct_origin_marker_fresh(entry: dict[str, Any]) -> bool:
    if _direct_origin_marker_expired(entry):
        ...
        return False
    if not str(entry.get("direct_origin_at") or entry.get("direct_origin_created_at") or ""):
        return False
    if not str(entry.get("direct_origin_user_text_hash") or "") or not _direct_origin_origin_allows(entry):
        _clear_direct_origin_marker(entry)
        return False
    return True


def _direct_origin_marker_same_pane(entry: dict[str, Any], pane: dict[str, Any] | None = None) -> bool:
    marker_pane_id = str(entry.get("direct_origin_pane_id") or "").strip()
    marker_pane_key = str(entry.get("direct_origin_pane_key") or "").strip()
    pane = pane or {}
    pane_id = str(pane.get("pane_id") or entry.get("pane_id") or "").strip()
    pane_key_value = str(pane.get("pane_key") or entry.get("pane_key") or "").strip()
    if marker_pane_id and pane_id and marker_pane_id != pane_id:
        return False
    if marker_pane_key and pane_key_value and marker_pane_key != pane_key_value:
        return False
    return bool((not marker_pane_id or pane_id) and (not marker_pane_key or pane_key_value))


def _direct_origin_after_turn_id(entry: dict[str, Any], pane_id: str = "") -> str:
    clean_pane_id = str(pane_id or "").strip()
    if clean_pane_id and not entry_is_tendwire_source(entry):
        try:
...
def mark_direct_origin_send(
    ...
    *,
    ...
    user_hash = _direct_origin_user_text_hash(outbound)
    clean_origin = str(origin or "").strip().lower()
    if not outbound or not user_hash or not _direct_origin_origin_allows(clean_origin):
        return False
    clean_pane_id = sanitize_text(str(pane_id or entry.get("pane_id") or ""), 300).strip()
    clean_pane_key = sanitize_text(str(pane_key_value or entry.get("pane_key") or ""), 300).strip()
    created_at = utc_now()
    values = {
        "direct_origin_at": created_at,
        "direct_origin_created_at": created_at,
        "direct_origin_origin": clean_origin,
        "direct_origin_pane_id": clean_pane_id,
        "direct_origin_pane_key": clean_pane_key,
        "direct_origin_user_text_hash": user_hash,
        "direct_origin_text_hash": user_hash,
        "direct_origin_after_turn_id": sanitize_text(str(after_turn_id or ""), 300).strip()
        or _direct_origin_after_turn_id(entry, clean_pane_id),
    }
    changed = _clear_direct_origin_marker(entry)
...
def bind_direct_origin_turn(
    ...
    user_hash = _direct_origin_user_text_hash(user_text)
    if not clean_turn_id or not user_hash or not _direct_origin_marker_fresh(entry):
        return False
    if not _direct_origin_marker_same_pane(entry, pane):
        _clear_direct_origin_marker(entry)
        return False
    if str(entry.get("direct_origin_user_text_hash") or "") != user_hash:
        _clear_direct_origin_marker(entry)
        return False
    bound_turn_id = sanitize_text(str(entry.get("direct_origin_turn_id") or ""), 300).strip()
    if bound_turn_id:
        if bound_turn_id != clean_turn_id:
            _clear_direct_origin_marker(entry)
            return False
        return str(entry.get("direct_origin_consumed_turn_id") or "") != clean_turn_id
    if str(entry.get("direct_origin_consumed_turn_id") or ""):
        return False
...
def mark_direct_origin_progress_consumed(
    entry: dict[str, Any],
    turn_id: str,
    user_text_hash: str,
    stream_hash: str = "",
    ...
    clean_turn_id = sanitize_text(str(turn_id or ""), 300).strip()
    clean_user_hash = str(user_text_hash or "").strip()
    if not clean_turn_id or not clean_user_hash or not _direct_origin_marker_fresh(entry):
        return False
    entry["direct_origin_consumed_turn_id"] = clean_turn_id
    entry["direct_origin_consumed_user_text_hash"] = clean_user_hash
    if stream_hash:
        entry["direct_origin_consumed_hash"] = str(stream_hash or "").strip()
    else:
        entry.pop("direct_origin_consumed_hash", None)
...
def retire_direct_origin_marker(entry: dict[str, Any], turn_id: str = "") -> bool:
    ...
    if clean_turn_id in {
        ...
        return _clear_direct_origin_marker(entry)
    if _direct_origin_marker_fresh(entry) and clean_turn_id != str(entry.get("direct_origin_after_turn_id") or ""):
        return _clear_direct_origin_marker(entry)
    return False
...
def extract_turn_feed_item(
    ...
    direct_origin_user_text = prompt_text
    if turn.get("complete") is False:
        direct_origin_open_turn_id = str(turn.get("turn_id") or "")
        direct_origin_user_text = direct_origin_user_text or str(turn.get("user_text") or "")
    elif turn.get("has_open_turn") is True:
        direct_origin_open_turn_id = str(turn.get("open_turn_id") or turn.get("turn_id") or "")
        direct_origin_user_text = direct_origin_user_text or str(turn.get("open_user_text") or turn.get("user_text") or "")
    if direct_origin_open_turn_id:
        bind_direct_origin_turn(entry, direct_origin_open_turn_id, direct_origin_user_text, pane)
...
def clear_stream_state(entry: dict[str, Any]) -> None:
    pending_turn_id = str(entry.get("pending_stream_turn_id") or "")
    if pending_turn_id:
        retire_direct_origin_marker(entry, pending_turn_id)
    for key in (
...
def pinned_status_pane_label(state: dict[str, Any], pane: dict[str, Any]) -> str:
    ...
    council_label = council_display_label_for_entry_like(entry if isinstance(entry, dict) else {}, pane)
    if council_label:
        return council_label
    raw = ""
    if isinstance(entry, dict):
...
def interrupt_and_send_response(
    ...
    *,
    ...
    if not ok:
        return {"handled": True, "reply": f"Send failed: {sanitize_text(detail, 300)}"}
    if entry is not None and mark_direct_origin_send(entry, outbound, after_turn_id=after_turn_id, origin="send_force", pane_id=pane_id) and state is not None:
        save_state(state)
    if detail:
...
def ensure_pane_entry(state: dict[str, Any], pane: dict[str, Any]) -> tuple[str, dict[str, Any], bool]:
    ...
    if source == "tendwire":
        entry["entry_type"] = str(pane.get("entry_type") or "worker")
        entry["worker_id"] = str(pane.get("worker_id") or pane.get("_tendwire_worker_id") or "")
        entry["worker_fingerprint"] = str(pane.get("worker_fingerprint") or pane.get("_tendwire_fingerprint") or "")
    else:
        entry.pop("entry_type", None)
        entry.pop("worker_id", None)
        entry.pop("worker_fingerprint", None)
...
def prune_closed_tendwire_source_records(
    ...
    *,
    ...
    """Drop closed Tendwire source records so source mode cannot recreate stale topics."""
    panes = state.get("panes") if isinstance(state.get("panes"), dict) else {}
    removed: list[dict[str, Any]] = []
    for key, entry in list(panes.items()):
        if not isinstance(entry, dict):
...
def pane_root_message_text(pane: dict[str, Any], entry: dict[str, Any]) -> str:
    council_label = council_display_label_for_entry_like(entry, pane)
    title = council_label or str(entry.get("pane_thread_name") or pane_thread_name(pane))
    pane_id = str(pane.get("pane_id") or entry.get("pane_id") or "")
    worker_id = str(pane.get("worker_id") or entry.get("tendwire_worker_id") or "")
    agent = council_label or str(pane.get("agent") or "")
    cwd = compact_path(pane.get("cwd") or pane.get("foreground_cwd") or "")
    lines = [
        f"Pane thread: {title}",
...
def pane_root_message_html(pane: dict[str, Any], entry: dict[str, Any]) -> str:
    title = html.escape(council_display_label_for_entry_like(entry, pane) or str(entry.get("pane_thread_name") or pane_thread_name(pane)))
...
def ensure_space_topic(
    ...
    onboarding_suppressed = str(space_entry.get("origin") or "").lower() == "council" and str(space_entry.get("voice_mode") or "") == "per_agent"
    if not per_agent_topics_enabled() and not onboarding_suppressed:
...
def route_message_to_pane(
    ...
    for space in spaces.values():
        ...
        for pane_key_value in space.get("pane_keys") or []:
            entry = panes.get(str(pane_key_value))
            if entry_is_live_route_target(entry) and str(entry.get("pane_root_message_id") or "") == message_key:
...
def _sync_pane_clean_feed(
    ...
    *,
    ...
    item = None
    if source_read_pane and TURN_FEED_ENABLED:
        before_turn_state = (
            entry.get("last_turn_available"),
            entry.get("last_turn_reason"),
            entry.get("last_turn_id"),
        )
        item = tendwire_source_turn_feed_item(pane, entry)
        after_turn_state = (
            entry.get("last_turn_available"),
            entry.get("last_turn_reason"),
            entry.get("last_turn_id"),
        ...
        not source_read_pane
        and not turn_only
        and str(pane.get("agent_status") or "").strip().lower() not in ACTIVE_AGENT_STATUSES
    ):
    ...
    if not source_read_pane and not item and TURN_FEED_ENABLED:
    ...
    pending_stream_user_text_hash = _direct_origin_user_text_hash(pending_stream_user_text)
    ...
    if (
        ...
        and pending_stream_user_text_hash
        ...
        and _direct_origin_origin_allows(entry)
        and _direct_origin_marker_same_pane(entry, pane)
        and str(entry.get("direct_origin_turn_id") or "") == pending_stream_turn_id
        and str(entry.get("direct_origin_user_text_hash") or "") == pending_stream_user_text_hash
    ):
        ...
        already_consumed = (
            ...
            and str(entry.get("direct_origin_consumed_user_text_hash") or "") == pending_stream_user_text_hash
        )
        if not already_consumed:
            ...
            if stream_result.get("ok"):
                ...
                if stream_result.get("sent_message"):
                    counters["sends"] = counters.get("sends", 0) + 1
                    counters["feed_sends"] = counters.get("feed_sends", 0) + 1
                if not stream_result.get("skipped"):
                    mark_direct_origin_progress_consumed(
                        entry,
                        pending_stream_turn_id,
                        pending_stream_user_text_hash,
                        fallback_stream_hash,
                    )
                changed = True
...
def entry_is_live_route_target(entry: dict[str, Any] | None) -> bool:
    if not isinstance(entry, dict):
        return False
...
def preferred_active_route_entry(
    ...
    active_statuses = {"active", "busy", "in_progress", "pending", "running", "working"}
    active = [
        item
        for item in entries
        if str(item[1].get("last_known_status") or "").strip().lower().replace("-", "_") in active_statuses
    ]
    if len(active) == 1:
        return active[0]
    return None
...
def resolve_topic_entry(
    ...
    *,
    ...
    if mapped_space:
        ...
        if clean_target_bot_kind:
            preferred = preferred_active_route_entry(live_entries)
    ...
    closed_entries: list[tuple[str, dict[str, Any]]] = []
    for entry in (state.get("panes") or {}).values():
...
def command_reply(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(attachment, dict) and attachment.get("kind") in {"document", "photo"} and attachment.get("file_id"):
        ...
        if mark_direct_origin_send(entry, instruction, after_turn_id=after_turn_id, origin="attachment", pane_id=pane_id):
            save_state(state)
    ...
    if command == "plain":
        ...
        if awaiting and str(awaiting.get("user_id") or "") == user_id:
            ...
            entry.pop("awaiting_detail", None)
            entry.pop("active_prompt", None)
            mark_direct_origin_send(entry, direct_origin_detail_text, after_turn_id=direct_origin_after_turn_id, origin="choice_detail", pane_id=pane_id)
            save_state(state)
            return {"handled": True, "reply": "Sent details."}
...
def callback_reply(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    entry.pop("active_prompt", None)
    entry.pop("awaiting_detail", None)
    mark_direct_origin_send(entry, outbound, after_turn_id=after_turn_id, origin="choice", pane_id=pane_id)
    save_state(state)
...
def doctor_once(args: Any) -> dict[str, Any]:
    load_dotenv()
    legacy_timer = legacy_topic_timer_doctor(fix=bool(getattr(args, "fix", False)))
    return {
        "ok": bool(legacy_timer.get("ok")),
        "checks": {
